...
-- Profiles: cada usuário vê/edita apenas o próprio
create policy "profiles_select_own" on profiles for select using (auth.uid() = id);
create policy "profiles_update_own" on profiles for update using (auth.uid() = id);

-- Addresses: apenas o dono
create policy "addresses_all_own" on addresses using (auth.uid() = user_id);

-- Products: todos podem ler; apenas admins escrevem
...
-- Order items: acompanham o pedido
